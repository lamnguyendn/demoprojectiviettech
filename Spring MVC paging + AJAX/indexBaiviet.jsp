				...
				<table id="myTable" class="table-hover table-bordered table-striped" width ="100%">
					...
					<tbody>
						...
						<tr>
							...
							<td class="edit_active text-center"id="setactive-<%=bv.getId_baiviet()%>">
								<a href="javascript:void(0)" onclick="return setActive(<%=bv.getId_baiviet()%>)" title="Kích hoạt">
									<%if(bv.isActive()){ %>
										<img src="<%=request.getContextPath() %>/templates/admin/images/tick-circle.gif" alt="" />
									<%} else { %>
										<img src="<%=request.getContextPath() %>/templates/admin/images/minus-circle.gif">
									<%} %>
								</a>
